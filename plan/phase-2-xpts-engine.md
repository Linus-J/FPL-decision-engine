...
- **Gate:** the **≥57 pts/GW** exit gate ran on the P-XI harness with P-RS scoring, MC seeded (deterministic) — **result: 42.64, gate FAILS.** The gap is now plausibly dominated by the structural free-tier-data ceiling (missing clearances/blocks/recoveries/crosses/dribbles/big-chances — the same fields behind both the DefCon underestimate and the bonus miscalibration) rather than by further fixable code bugs. Per the plan's own decision rule ("If below 57, iterate inside Phase 2 — open decision 3"), further iteration without richer event data has diminishing expected return; a paid/fuller event feed is the more likely unlock. Open decision for the user.
