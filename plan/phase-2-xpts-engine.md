...
- **Leading root-cause hypothesis (not yet fixed):** the captain pick was a goalkeeper in the large majority of GWs (Donnarumma/Roefs alternating) — a real red flag, since GK is almost never the correct FPL captain. Likely cause: P8's reduced-BPS approximation happens to preserve GK's primary BPS channel (saves) fully, while stripping out MANY of attackers'/midfielders' secondary BPS channels (crosses, dribbles, big-chances-created — genuinely absent from the free FBref feed, same root cause as the DefCon gap). A non-scoring attacker's match BPS collapses to ~appearance-only under the reduction, while a busy GK's BPS stays close to its real value — systematically over-crediting GK bonus-competitiveness relative to reality. Not yet investigated further or fixed.
- **Gate:** the **≥57 pts/GW** exit gate ran on the P-XI harness with P-RS scoring, MC seeded (deterministic) — **result: 41.97, gate FAILS.** Per the plan's own decision rule ("If below 57, iterate inside Phase 2 — open decision 3"), this is expected next-step signal, not a blocker to report as broken.
