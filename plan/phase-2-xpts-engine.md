...
- **European/cup congestion (rotation + fatigue).** Teams in UCL/UEL/UECL and deep in the domestic cups rotate their league XI (esp. Thu-UEL→weekend) and fade in congested blocks — a systematic minutes signal for fringe/rotation players. Add: a per-team "in Europe this season" flag + a rolling fixture-density feature (games in the last N days, from the team's full all-competitions fixture list — FBref carries UCL/UEL/UECL + FA/EFL Cup). This sharpens the "rotation/congestion" feature above rather than adding a pillar; 26/27 European participation is known pre-season from the 25/26 finish + cup outcomes.
